...
\newcommand{\srcVG}{VG~\cite{krishna2017visual,xu2017scene}}
\newcommand{\srcOI}{OI~\cite{kuznetsova2020openimages}}
\newcommand{\srcPSG}{PSG~\cite{yang2022psg}}
\newcommand{\srcGQA}{GQA~\cite{hudson2019gqa}}
\newcommand{\srcVRD}{VRD~\cite{lu2016vrd}}
\newcommand{\srcCRADIO}{C-RADIOv4~\cite{ranzinger2026cradiov4}}
\newcommand{\srcRADIO}{RADIOv2.5-B~\cite{heinrich2025radio25}}
\newcommand{\srcDINO}{DINOv2-B~\cite{oquab2024dinov2}}
\newcommand{\srcSigLIP}{SigLIP2-B~\cite{tschannen2025siglip2}}
\newcommand{\srcSAM}{SAM ViT-B~\cite{kirillov2023sam}}
\newcommand{\srcResNet}{ResNet-50~\cite{he2016resnet}}
\newcommand{\srcEGTR}{EGTR~\cite{im2024egtr}}
\newcommand{\srcRelTR}{RelTR~\cite{cong2023reltr}}
\newcommand{\srcSGTR}{SGTR~\cite{li2022sgtr}}
\newcommand{\srcKERN}{KERN~\cite{chen2019kern}}
\newcommand{\srcBGNN}{BGNN~\cite{li2021bgnn}}
\newcommand{\srcMotifs}{Motifs~\cite{zellers2018motifs}}
\newcommand{\srcVCTree}{VCTree~\cite{tang2019vctree}}
\newcommand{\srcPSGFormer}{PSGFormer~\cite{yang2022psg}}
\newcommand{\srcPSGTR}{PSGTR~\cite{yang2022psg}}
\newcommand{\srcTDEMotifs}{TDE-Motifs~\cite{tang2020unbiased}}
\newcommand{\srcPyMotifs}{PySGG Motifs~\cite{zellers2018motifs,li2021bgnn}}
\newcommand{\srcPyTransformer}{PySGG Transformer~\cite{li2021bgnn}}
...
\begin{abstract}
Scene graph generation (SGG) is summarized by triplet recall,
conflating localization, object recognition, and relation prediction. We
introduce \textsc{GroundedSGG-Bench}, a reproducible benchmark that separates
this chain through support-conditioned object probes,
PredCls/SGCls/SGDet evaluation, endpoint interventions, a motif-conditioned
audit, and object-head mitigation. VG-150, Open Images, and Panoptic Scene
Graph provide evaluation; GQA and VRD provide frozen exact-overlap
transfer diagnostics. Across six frozen visual backbones,
\keynum{13.6--58.5\%} of eligible relations contain an endpoint identity
disagreement even when both prompted masks reach intersection-over-union 0.85.
On the full VG test set, endpoint errors correspond to
\keynum{8.9--10.4}-point predicate-accuracy declines for three systems.
Masking task-relevant endpoints reduces predicate Top-1 by \keynum{23.3}
points for Motifs and \keynum{42.5} for a Transformer, more than
unrelated-node masking. A 20-motif audit shows that aggregate invariance
(86.2--88.9\%) is dominated by stable wrong predictions: clean-correct
persistence is only \keynum{1.05--1.74\%}, \keynum{51.3--57.9} points below
matched controls. Across 11 full-split SGDet runs from nine model families,
localization and grounded-object rates expose bottlenecks hidden by end-to-end
scores. Three-seed TDE-Motifs tuning improves object Top-1 by 0.48--0.50 points
and reduces ECE from 17.87\% to 2.68--3.22\%, but has no stable grounding
advantage over supervised control; the selected checkpoint also reduces SGDet
mR@50 by \keynum{6.35} points. Overall, \textsc{GroundedSGG-Bench} provides a
systematic framework for separating spatial support, visual identity, and
relation reasoning quality, avoiding misleading comparisons caused by
collapsing heterogeneous datasets and evaluation settings into a single
ranking.
\end{abstract}
...
\begin{center}
\footnotesize
\textbf{Anonymous artifact:}
\href{https://anonymous.4open.science/r/kdd2027-submission-v1-CA7D/README.md}
{\nolinkurl{anonymous.4open.science/r/kdd2027-submission-v1-CA7D/README.md}}
\end{center}
\vspace{-0.5em}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.78\textwidth]{benchmark_overview.pdf}
...
\midrule
I-A & Does accurate support imply decodable object identity? & \srcPSG; six frozen backbones; box, GT-mask, box-prompted SAM views & Top-1/5, macro, head/body/tail, ECE, high-IoU endpoint disagreement & Scoped to GT-box-prompted support and object probes \\
I-B & How does relation depth change fixed features? & \srcVG\ PredCls component model & R/mR/zR, effective rank, Dirichlet energy & Relation-only scope under GT object labels \\
II-A & How does predicate accuracy vary with endpoint-label agreement? & Full official prediction caches on \srcVG, \srcOI, and \srcPSG & Hit@1 by endpoint-agreement stratum; support, coverage, and CI & Observational association only \\
II-B & Do targeted endpoint interventions alter relation predictions? & \srcMotifs\ and \srcPyTransformer; five strengths; matched controls & Dose--response Hit@1 and matched-control contrasts & Intervention-specific, not a universal causal model \\
III & Do frequent motif predictions persist after endpoint evidence is removed? & \srcMotifs\ and \srcPyTransformer; train-mined motifs; GT relation pairs & PIR, clean-correct MAR, clean-wrong WSR, matched-control deltas & Motif-conditioned intervention; no full causal identification \\
IV & Are standard and grounding metrics reproducible across families? & Broad native SGDet panel; two-family matched \srcVG\ tri-task panel & PredCls/SGCls/SGDet R/mR/zR; decomposition & Breadth and matched-depth results are separate \\
V & Can task-focused object calibration improve identity without harming relations? & \srcTDEMotifs; two modes; three seeds; frozen relation head & Object Top-1/ECE, SGCls compatibility, full selected-checkpoint test, \srcGQA/\srcVRD\ transfer & One-family mitigation; grounding benefit is tested against its control \\
\bottomrule
\end{tabularx}
\end{table*}
...
\midrule
VG-150 \cite{krishna2017visual,xu2017scene} & Main standard & boxes & 50 predicates & PredCls/SGCls/SGDet & Established closed-vocabulary comparison \\
...
\paragraph{Recommended reporting unit.}
To prevent the benchmark from becoming a single cross-dataset leaderboard,
each result binds the dataset and ontology, task, support definition, identity
metric, relation metric, coverage, and asset provenance. Native comparisons
are restricted to compatible dataset blocks; transfer claims use only the
registered exact-overlap diagnostic. This unit distinguishes missing support
from measured failure and makes apparent gains traceable to localization,
identity, relation reasoning, or calibration.
...
\midrule
\srcCRADIO & \keynum{$77.95\pm0.13$} & \keynum{$81.17\pm0.04$} & \keynum{$76.27\pm0.15$} & \keynum{$2.63\pm0.22$} & \keynum{$13.57\pm0.55$} \\
\srcDINO & $75.51\pm0.10$ & $79.05\pm0.10$ & $72.92\pm0.22$ & $2.74\pm0.10$ & $17.12\pm0.19$ \\
\srcRADIO & $72.88\pm0.13$ & $76.75\pm0.11$ & $69.69\pm0.16$ & $3.21\pm0.15$ & $17.69\pm0.33$ \\
\srcSigLIP & $62.52\pm0.08$ & $66.47\pm0.17$ & $58.36\pm0.14$ & $5.20\pm0.08$ & $23.91\pm0.14$ \\
\srcSAM & $54.66\pm0.08$ & $58.24\pm0.03$ & $39.51\pm0.16$ & $4.51\pm0.04$ & $48.80\pm0.31$ \\
\srcResNet & $39.77\pm0.13$ & $41.66\pm0.20$ & $30.65\pm0.14$ & $12.11\pm0.27$ & \keynum{$58.48\pm0.77$} \\
\bottomrule
\end{tabular}
\end{table*}
...
A detailed comparison with contextual, debiased, and end-to-end SGG appears in
Appendix~\ref{sec:related-work}. GroundedSGG-Bench does not claim universal
causal attribution or preserved detector utility: PredCls supplies labels,
prompted masks provide oracle support, and external transfer uses exact
vocabulary overlap. We redistribute no images. Scene graphs inherit dataset
bias and should not support surveillance or high-stakes decisions without
separate consent, privacy, and fairness review. Extended limitations and
resource costs also appear in the appendix.
...
and relation quality rather than a single end-to-end score. The anonymous
reviewer artifact provides the experiment contracts, manifests, and
one-command reproduction entry points used here while retaining upstream data
and checkpoints under their original terms; it is available at
\href{https://anonymous.4open.science/r/kdd2027-submission-v1-CA7D/}{the
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{appendix_result_summary.pdf}
  \caption{Compact visual summary of the later benchmark stages. (a)
  Experiment IV decomposes each SGDet run into localization, recognition
  conditional on localization, and the resulting grounded-object rate. (b)
  Experiment V shows validation object Top-1 gain against
  post-tuning ECE for all six TDE-Motifs runs; filled markers pass the
  registered validation gate. (c) Frozen exact-overlap transfer reports the
  range over six states and marks the median gain for object and triplet
  accuracy. Dataset-specific evaluation and transfer coverage remain as
  reported in the formal tables.}
  \Description{Three panels visualize SGDet localization versus conditional
  recognition, validation calibration trade-offs for six mitigation runs, and
  external object and triplet accuracy gains on GQA and VRD.}
  \label{fig:app-result-summary}
\end{figure*}
...
\midrule
I-A & \srcPSG\ object probe & 65,102 train / 11,667 eval objects & 6 backbones, 3 views, 3 seeds & \ref{tab:app-exp1a-complete}--\ref{tab:app-exp1a-endpoints} \\
I-B & \srcVG\ PredCls & 2,000 test images & 6 backbones, 4 depths, 3 seeds & \ref{tab:app-exp1b-r}--\ref{tab:app-exp1b-diagnostics} \\
II-A & \srcVG/\srcOI/\srcPSG\ observational & Full registered splits & 8 task runs / 7 model--dataset runs & \ref{tab:app-exp2-coverage}--\ref{tab:app-exp2-groups} \\
II-B & \srcVG\ controlled interventions & 1,999/2,000 images, 3 seeds & \srcMotifs; \srcPyTransformer & \ref{tab:app-exp2b-dose}--\ref{tab:app-exp2b-endpoints} \\
III & \srcVG\ motif terminal audit & 20 motifs / 1,411 GT pairs & \srcMotifs; \srcPyTransformer & \ref{tab:app-exp3-motif} \\
IV & Native SGDet & 26,446/1,813/1,000 images & 11 runs, 9 families; 9 decomposed & \ref{tab:app-exp4-r}--\ref{tab:app-exp4-grounding} \\
IV-depth & \srcVG\ PredCls/SGCls/SGDet & 26,446 images & \srcMotifs; \srcPyTransformer & \ref{tab:app-exp4-depth-metrics}--\ref{tab:app-exp4-depth-reference} \\
IV auxiliary & \srcVG\ SGCls & 26,446 images & \srcKERN & \ref{tab:app-kern-sgcls} \\
V & Object-head mitigation & 5,000 train / 1,000 validation images; 2 modes; 3 seeds & \srcTDEMotifs & \ref{tab:app-exp5-vg}--\ref{tab:app-exp5-test} \\
V external & Frozen shared-\srcVG\ transfer & \srcGQA\ 604/1,000; \srcVRD\ 770/955 retained images & Six selected \srcTDEMotifs\ states & \ref{tab:app-exp5-external} \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
\srcCRADIO & Box & $77.95\pm0.13$ & $96.05\pm0.02$ & $71.71\pm0.04$ & $3.67\pm0.27$ & $0.826\pm0.001$ & $0.320\pm0.001$ \\
\srcCRADIO & SAM mask & \keynum{$81.17\pm0.04$} & $96.14\pm0.08$ & \keynum{$76.27\pm0.15$} & $2.63\pm0.22$ & $0.728\pm0.005$ & $0.278\pm0.002$ \\
\srcCRADIO & GT mask & \keynum{$86.54\pm0.03$} & \keynum{$97.52\pm0.04$} & \keynum{$81.79\pm0.11$} & $1.90\pm0.17$ & \keynum{$0.521\pm0.001$} & \keynum{$0.200\pm0.000$} \\
\srcDINO & Box & $75.51\pm0.10$ & $95.10\pm0.09$ & $69.09\pm0.11$ & $3.87\pm0.14$ & $0.937\pm0.005$ & $0.354\pm0.002$ \\
\srcDINO & SAM mask & $79.05\pm0.10$ & $95.51\pm0.04$ & $72.92\pm0.22$ & $2.74\pm0.10$ & $0.806\pm0.001$ & $0.304\pm0.000$ \\
\srcDINO & GT mask & $84.66\pm0.03$ & $96.92\pm0.02$ & $78.98\pm0.13$ & $1.77\pm0.25$ & $0.600\pm0.003$ & $0.225\pm0.001$ \\
\srcRADIO & Box & $72.88\pm0.13$ & $93.90\pm0.04$ & $65.07\pm0.15$ & $3.66\pm0.16$ & $1.037\pm0.003$ & $0.389\pm0.001$ \\
\srcRADIO & SAM mask & $76.75\pm0.11$ & $94.50\pm0.06$ & $69.69\pm0.16$ & $3.21\pm0.15$ & $0.900\pm0.002$ & $0.337\pm0.001$ \\
\srcRADIO & GT mask & $82.16\pm0.12$ & $96.05\pm0.04$ & $75.32\pm0.16$ & $2.24\pm0.06$ & $0.697\pm0.002$ & $0.263\pm0.001$ \\
\srcSigLIP & Box & $62.52\pm0.08$ & $87.73\pm0.08$ & $54.72\pm0.02$ & $5.95\pm0.15$ & $1.550\pm0.001$ & $0.533\pm0.000$ \\
\srcSigLIP & SAM mask & $66.47\pm0.17$ & $88.26\pm0.04$ & $58.36\pm0.14$ & $5.20\pm0.08$ & $1.441\pm0.003$ & $0.486\pm0.001$ \\
\srcSigLIP & GT mask & $69.80\pm0.06$ & $89.43\pm0.11$ & $61.52\pm0.17$ & $4.90\pm0.16$ & $1.321\pm0.001$ & $0.440\pm0.000$ \\
\srcSAM & Box & $54.66\pm0.08$ & $80.74\pm0.02$ & $35.87\pm0.16$ & $4.84\pm0.11$ & $1.914\pm0.001$ & $0.606\pm0.000$ \\
\srcSAM & SAM mask & $58.24\pm0.03$ & $81.79\pm0.08$ & $39.51\pm0.16$ & $4.51\pm0.04$ & $1.782\pm0.004$ & $0.562\pm0.001$ \\
\srcSAM & GT mask & $62.17\pm0.11$ & $85.07\pm0.03$ & $43.15\pm0.02$ & $4.07\pm0.11$ & $1.600\pm0.002$ & $0.511\pm0.001$ \\
\srcResNet & Box & $39.77\pm0.13$ & $71.41\pm0.06$ & $29.42\pm0.23$ & $11.13\pm0.14$ & $2.762\pm0.001$ & $0.805\pm0.000$ \\
\srcResNet & SAM mask & $41.66\pm0.20$ & $71.86\pm0.11$ & $30.65\pm0.14$ & $12.11\pm0.27$ & $2.746\pm0.002$ & $0.792\pm0.001$ \\
\srcResNet & GT mask & $43.17\pm0.14$ & $72.66\pm0.22$ & $31.88\pm0.20$ & $12.82\pm0.33$ & $2.705\pm0.003$ & $0.782\pm0.000$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
\srcCRADIO & $79.38\pm0.20$ & $76.22\pm0.58$ & $72.99\pm0.12$ & $83.51\pm0.09$ & $84.72\pm0.21$ & $76.39\pm0.07$ \\
\srcDINO & $76.26\pm0.20$ & $74.63\pm0.49$ & $67.57\pm0.79$ & $80.37\pm0.14$ & $82.89\pm0.16$ & $75.37\pm0.09$ \\
\srcRADIO & $73.69\pm0.08$ & $70.69\pm0.31$ & $64.35\pm0.26$ & $76.64\pm0.44$ & $80.43\pm0.37$ & $74.88\pm0.18$ \\
\srcSigLIP & $61.99\pm0.13$ & $61.87\pm0.16$ & $50.77\pm0.48$ & $59.51\pm0.18$ & $73.16\pm0.63$ & $71.27\pm0.05$ \\
\srcSAM & $50.74\pm0.15$ & $40.22\pm0.28$ & $26.74\pm0.28$ & $58.54\pm0.19$ & $57.75\pm0.15$ & $58.13\pm0.27$ \\
\srcResNet & $35.86\pm0.53$ & $30.85\pm0.16$ & $24.86\pm0.23$ & $32.00\pm0.40$ & $43.28\pm0.25$ & $52.52\pm0.20$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
\srcCRADIO & $47.22\pm0.25$ & $79.33\pm0.15$ & $87.87\pm0.16$ & $91.53\pm0.12$ & $92.52\pm0.08$ \\
\srcDINO & $45.12\pm0.32$ & $77.09\pm0.44$ & $85.64\pm0.16$ & $89.39\pm0.04$ & $91.62\pm0.21$ \\
\srcRADIO & $43.77\pm0.28$ & $73.51\pm0.36$ & $82.65\pm0.20$ & $87.73\pm0.22$ & $91.57\pm0.14$ \\
\srcSigLIP & $35.67\pm0.26$ & $58.87\pm0.46$ & $70.35\pm0.06$ & $80.05\pm0.19$ & $87.35\pm0.14$ \\
\srcSAM & $31.88\pm0.15$ & $53.97\pm0.22$ & $63.82\pm0.12$ & $67.03\pm0.17$ & $70.55\pm0.56$ \\
\srcResNet & $22.67\pm0.18$ & $32.25\pm0.14$ & $42.26\pm0.42$ & $52.29\pm0.11$ & $69.48\pm0.60$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
\srcCRADIO & $15.31\pm0.35$ & \keynum{$13.57\pm0.55$} & $14.06\pm1.19$ & $12.78\pm0.79$ \\
\srcDINO & $17.80\pm0.24$ & $17.12\pm0.19$ & $17.12\pm0.38$ & $7.22\pm0.79$ \\
\srcRADIO & $19.77\pm0.23$ & $17.69\pm0.33$ & $17.02\pm0.50$ & $10.56\pm0.79$ \\
\srcSigLIP & $27.14\pm0.11$ & $23.91\pm0.14$ & $21.63\pm0.08$ & $20.00\pm2.72$ \\
\srcSAM & $50.80\pm0.28$ & $48.80\pm0.31$ & $49.33\pm0.27$ & $36.67\pm2.36$ \\
\srcResNet & $63.24\pm0.71$ & \keynum{$58.48\pm0.77$} & $55.82\pm1.33$ & $47.78\pm2.08$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
\srcCRADIO & 0 & $2.52\pm0.17$ & $9.45\pm0.10$ & $14.89\pm0.13$ & $22.28\pm0.09$ & $35.08\pm0.05$ & $45.92\pm0.15$ \\
\srcCRADIO & 2 & $4.02\pm0.11$ & $13.31\pm0.22$ & $19.43\pm0.51$ & $26.71\pm0.17$ & $39.26\pm0.26$ & $49.67\pm0.25$ \\
\srcCRADIO & 4 & $3.95\pm0.26$ & $13.39\pm0.65$ & $19.99\pm0.65$ & $27.97\pm0.54$ & $40.71\pm0.68$ & $51.63\pm0.69$ \\
\srcCRADIO & 8 & $3.61\pm0.08$ & $12.57\pm0.36$ & $19.01\pm0.22$ & $27.01\pm0.38$ & $40.22\pm0.53$ & $51.39\pm0.47$ \\
\srcDINO & 0 & $2.62\pm0.11$ & $9.38\pm0.17$ & $15.05\pm0.28$ & $22.27\pm0.19$ & $35.22\pm0.31$ & $45.70\pm0.12$ \\
\srcDINO & 2 & $4.03\pm0.11$ & $13.01\pm0.23$ & $19.26\pm0.11$ & $26.34\pm0.22$ & $38.38\pm0.12$ & $48.84\pm0.49$ \\
\srcDINO & 4 & $4.01\pm0.13$ & $13.30\pm0.24$ & $19.60\pm0.33$ & $27.17\pm0.41$ & $39.36\pm0.47$ & $49.89\pm0.17$ \\
\srcDINO & 8 & $3.79\pm0.18$ & $12.75\pm0.39$ & $19.37\pm0.60$ & $26.97\pm0.78$ & $39.78\pm0.92$ & $50.64\pm1.04$ \\
\srcRADIO & 0 & $2.80\pm0.18$ & $10.16\pm0.03$ & $15.83\pm0.10$ & $23.87\pm0.14$ & $37.69\pm0.13$ & $49.38\pm0.23$ \\
\srcRADIO & 2 & $4.07\pm0.05$ & $13.48\pm0.21$ & $20.08\pm0.19$ & $27.41\pm0.18$ & $39.50\pm0.10$ & $50.14\pm0.40$ \\
\srcRADIO & 4 & $4.25\pm0.25$ & $13.77\pm0.61$ & $20.29\pm0.86$ & $28.11\pm0.76$ & $40.46\pm0.61$ & $50.48\pm0.89$ \\
\srcRADIO & 8 & $3.99\pm0.01$ & $13.24\pm0.09$ & $19.87\pm0.38$ & $28.32\pm0.15$ & $42.09\pm0.32$ & $53.18\pm0.14$ \\
\srcSigLIP & 0 & $2.47\pm0.11$ & $9.49\pm0.01$ & $15.00\pm0.12$ & $22.11\pm0.06$ & $34.92\pm0.05$ & $45.87\pm0.07$ \\
\srcSigLIP & 2 & $4.15\pm0.10$ & $13.60\pm0.22$ & $19.99\pm0.15$ & $27.10\pm0.25$ & $39.24\pm0.18$ & $49.32\pm0.31$ \\
\srcSigLIP & 4 & $4.06\pm0.14$ & $13.28\pm0.45$ & $19.75\pm0.38$ & $27.38\pm0.43$ & $40.20\pm0.77$ & $50.72\pm0.48$ \\
\srcSigLIP & 8 & $3.75\pm0.20$ & $12.83\pm0.32$ & $19.26\pm0.47$ & $26.86\pm0.39$ & $40.21\pm0.51$ & $51.02\pm0.37$ \\
\srcSAM & 0 & $2.36\pm0.06$ & $9.20\pm0.19$ & $15.19\pm0.27$ & $23.10\pm0.36$ & $36.92\pm0.08$ & $49.23\pm0.18$ \\
\srcSAM & 2 & $3.98\pm0.28$ & $14.11\pm0.28$ & $21.15\pm0.59$ & $29.43\pm0.22$ & $42.99\pm0.30$ & $54.51\pm0.14$ \\
\srcSAM & 4 & $4.08\pm0.13$ & $14.16\pm0.11$ & $21.73\pm0.25$ & $29.90\pm0.74$ & $42.77\pm1.37$ & $53.21\pm2.47$ \\
\srcSAM & 8 & $3.83\pm0.26$ & $13.50\pm0.35$ & $20.60\pm0.22$ & $28.93\pm0.16$ & $42.24\pm0.29$ & $52.87\pm0.44$ \\
\srcResNet & 0 & $2.44\pm0.13$ & $8.51\pm0.17$ & $13.80\pm0.13$ & $21.24\pm0.31$ & $33.29\pm0.38$ & $44.37\pm0.39$ \\
\srcResNet & 2 & $3.62\pm0.07$ & $12.26\pm0.17$ & $18.10\pm0.21$ & $25.24\pm0.35$ & $37.38\pm0.76$ & $47.57\pm0.83$ \\
\srcResNet & 4 & $3.43\pm0.10$ & $12.05\pm0.13$ & $18.02\pm0.26$ & $25.62\pm0.24$ & $37.43\pm0.19$ & $47.63\pm0.19$ \\
\srcResNet & 8 & $3.34\pm0.09$ & $11.71\pm0.51$ & $17.53\pm0.46$ & $25.23\pm0.37$ & $37.36\pm0.46$ & $48.10\pm0.30$ \\
\bottomrule
\end{tabular}
\end{table*}
...
Backbone & Depth & @1 & @5 & @10 & @20 & @50 & @100 \\
\midrule
\srcCRADIO & 0 & $0.15\pm0.03$ & $0.62\pm0.04$ & $1.07\pm0.02$ & $1.85\pm0.04$ & $3.63\pm0.08$ & $5.63\pm0.12$ \\
\srcCRADIO & 2 & $0.26\pm0.02$ & $1.04\pm0.06$ & $1.64\pm0.09$ & $2.52\pm0.16$ & $4.41\pm0.23$ & $6.73\pm0.17$ \\
\srcCRADIO & 4 & $0.25\pm0.02$ & $1.04\pm0.07$ & $1.72\pm0.11$ & $2.73\pm0.14$ & $4.81\pm0.22$ & $7.27\pm0.27$ \\
\srcCRADIO & 8 & $0.23\pm0.02$ & $0.96\pm0.06$ & $1.62\pm0.08$ & $2.60\pm0.10$ & $4.72\pm0.04$ & $7.26\pm0.27$ \\
\srcDINO & 0 & $0.16\pm0.01$ & $0.60\pm0.02$ & $1.08\pm0.04$ & $1.93\pm0.05$ & $3.72\pm0.10$ & $5.65\pm0.10$ \\
\srcDINO & 2 & $0.24\pm0.01$ & $0.94\pm0.07$ & $1.52\pm0.07$ & $2.43\pm0.07$ & $4.33\pm0.03$ & $6.49\pm0.09$ \\
\srcDINO & 4 & $0.25\pm0.01$ & $0.99\pm0.06$ & $1.59\pm0.06$ & $2.55\pm0.12$ & $4.61\pm0.07$ & $6.84\pm0.12$ \\
\srcDINO & 8 & $0.24\pm0.00$ & $0.96\pm0.01$ & $1.55\pm0.03$ & $2.47\pm0.10$ & $4.59\pm0.22$ & $6.99\pm0.26$ \\
\srcRADIO & 0 & $0.18\pm0.01$ & $0.72\pm0.02$ & $1.23\pm0.02$ & $2.10\pm0.05$ & $3.98\pm0.05$ & $6.27\pm0.06$ \\
\srcRADIO & 2 & $0.24\pm0.01$ & $0.99\pm0.06$ & $1.63\pm0.08$ & $2.48\pm0.12$ & $4.34\pm0.12$ & $6.66\pm0.25$ \\
\srcRADIO & 4 & $0.26\pm0.02$ & $1.06\pm0.07$ & $1.70\pm0.10$ & $2.62\pm0.14$ & $4.58\pm0.18$ & $6.83\pm0.19$ \\
\srcRADIO & 8 & $0.22\pm0.01$ & $0.97\pm0.03$ & $1.63\pm0.02$ & $2.60\pm0.07$ & $4.88\pm0.08$ & $7.44\pm0.17$ \\
\srcSigLIP & 0 & $0.16\pm0.01$ & $0.63\pm0.02$ & $1.10\pm0.01$ & $1.85\pm0.02$ & $3.59\pm0.01$ & $5.54\pm0.06$ \\
\srcSigLIP & 2 & $0.25\pm0.01$ & $0.96\pm0.05$ & $1.58\pm0.09$ & $2.45\pm0.12$ & $4.39\pm0.16$ & $6.64\pm0.22$ \\
\srcSigLIP & 4 & $0.25\pm0.01$ & $0.99\pm0.03$ & $1.60\pm0.03$ & $2.54\pm0.06$ & $4.61\pm0.10$ & $7.02\pm0.11$ \\
\srcSigLIP & 8 & $0.23\pm0.01$ & $0.91\pm0.00$ & $1.50\pm0.05$ & $2.38\pm0.07$ & $4.59\pm0.18$ & $7.01\pm0.20$ \\
\srcSAM & 0 & $0.13\pm0.00$ & $0.56\pm0.01$ & $1.02\pm0.02$ & $1.80\pm0.01$ & $3.55\pm0.05$ & $5.79\pm0.03$ \\
\srcSAM & 2 & $0.21\pm0.02$ & $0.95\pm0.07$ & $1.59\pm0.03$ & $2.56\pm0.05$ & $4.85\pm0.06$ & $7.42\pm0.03$ \\
\srcSAM & 4 & $0.22\pm0.01$ & $0.97\pm0.04$ & $1.69\pm0.06$ & $2.70\pm0.16$ & $4.74\pm0.26$ & $7.22\pm0.41$ \\
\srcSAM & 8 & $0.20\pm0.01$ & $0.87\pm0.05$ & $1.49\pm0.08$ & $2.46\pm0.12$ & $4.49\pm0.12$ & $6.89\pm0.18$ \\
\srcResNet & 0 & $0.14\pm0.01$ & $0.58\pm0.02$ & $1.01\pm0.03$ & $1.74\pm0.06$ & $3.31\pm0.13$ & $5.24\pm0.19$ \\
\srcResNet & 2 & $0.21\pm0.00$ & $0.86\pm0.03$ & $1.41\pm0.02$ & $2.24\pm0.02$ & $4.13\pm0.07$ & $6.23\pm0.08$ \\
\srcResNet & 4 & $0.22\pm0.01$ & $0.85\pm0.03$ & $1.40\pm0.07$ & $2.30\pm0.06$ & $4.23\pm0.04$ & $6.35\pm0.10$ \\
\srcResNet & 8 & $0.20\pm0.01$ & $0.83\pm0.04$ & $1.40\pm0.01$ & $2.32\pm0.08$ & $4.22\pm0.03$ & $6.55\pm0.06$ \\
\bottomrule
\end{tabular}
\end{table*}
...
Backbone & Depth & @1 & @5 & @10 & @20 & @50 & @100 \\
\midrule
\srcCRADIO & 0 & $0.83\pm0.02$ & $4.05\pm0.17$ & $6.83\pm0.18$ & $11.03\pm0.06$ & $19.12\pm0.14$ & $26.63\pm0.53$ \\
\srcCRADIO & 2 & $1.45\pm0.10$ & $6.27\pm0.10$ & $9.66\pm0.33$ & $14.08\pm0.09$ & $23.19\pm0.25$ & $31.24\pm0.34$ \\
\srcCRADIO & 4 & $1.51\pm0.07$ & $6.23\pm0.22$ & $10.29\pm0.33$ & $15.19\pm0.41$ & $25.00\pm0.57$ & $33.24\pm0.81$ \\
\srcCRADIO & 8 & $1.28\pm0.14$ & $5.59\pm0.16$ & $9.78\pm0.14$ & $14.55\pm0.17$ & $24.39\pm0.27$ & $33.37\pm0.73$ \\
\srcDINO & 0 & $0.77\pm0.04$ & $3.58\pm0.11$ & $6.62\pm0.15$ & $10.95\pm0.30$ & $19.19\pm0.24$ & $26.00\pm0.47$ \\
\srcDINO & 2 & $1.63\pm0.08$ & $5.82\pm0.19$ & $9.27\pm0.16$ & $13.49\pm0.47$ & $22.38\pm0.47$ & $30.36\pm0.35$ \\
\srcDINO & 4 & $1.67\pm0.01$ & $5.63\pm0.30$ & $9.63\pm0.17$ & $14.36\pm0.68$ & $23.26\pm1.09$ & $31.38\pm0.71$ \\
\srcDINO & 8 & $1.51\pm0.18$ & $5.73\pm0.31$ & $9.38\pm0.36$ & $13.84\pm0.54$ & $23.73\pm0.68$ & $32.35\pm1.26$ \\
\srcRADIO & 0 & $0.91\pm0.09$ & $4.51\pm0.14$ & $7.39\pm0.22$ & $12.15\pm0.41$ & $21.21\pm0.19$ & $29.71\pm0.62$ \\
\srcRADIO & 2 & $1.50\pm0.06$ & $6.43\pm0.11$ & $10.34\pm0.40$ & $14.94\pm0.14$ & $23.61\pm0.53$ & $32.10\pm0.27$ \\
\srcRADIO & 4 & $1.53\pm0.10$ & $6.77\pm0.27$ & $10.57\pm0.45$ & $15.43\pm0.43$ & $24.64\pm0.26$ & $32.73\pm0.41$ \\
\srcRADIO & 8 & $1.45\pm0.13$ & $6.07\pm0.05$ & $10.27\pm0.23$ & $15.23\pm0.30$ & $25.79\pm0.26$ & $35.28\pm0.17$ \\
\srcSigLIP & 0 & $0.84\pm0.09$ & $4.34\pm0.34$ & $7.03\pm0.15$ & $11.21\pm0.30$ & $19.34\pm0.29$ & $26.69\pm0.15$ \\
\srcSigLIP & 2 & $1.75\pm0.04$ & $5.80\pm0.25$ & $9.40\pm0.27$ & $14.61\pm0.27$ & $23.19\pm0.33$ & $31.23\pm0.39$ \\
\srcSigLIP & 4 & $1.60\pm0.11$ & $6.00\pm0.47$ & $9.56\pm0.16$ & $14.82\pm0.22$ & $23.92\pm0.71$ & $32.69\pm1.00$ \\
\srcSigLIP & 8 & $1.58\pm0.03$ & $5.64\pm0.07$ & $9.22\pm0.10$ & $14.40\pm0.24$ & $24.72\pm0.62$ & $33.36\pm0.82$ \\
\srcSAM & 0 & $0.92\pm0.05$ & $4.03\pm0.09$ & $7.41\pm0.27$ & $12.18\pm0.48$ & $22.19\pm0.21$ & $30.94\pm0.54$ \\
\srcSAM & 2 & $1.48\pm0.05$ & $6.88\pm0.17$ & $11.59\pm0.58$ & $17.06\pm0.51$ & $27.99\pm0.94$ & $37.18\pm0.74$ \\
\srcSAM & 4 & $1.54\pm0.13$ & $6.71\pm0.20$ & $12.33\pm0.24$ & $17.77\pm0.54$ & $27.86\pm0.78$ & $36.50\pm1.17$ \\
\srcSAM & 8 & $1.50\pm0.04$ & $6.22\pm0.09$ & $10.58\pm0.34$ & $16.52\pm0.16$ & $27.29\pm0.50$ & $36.68\pm0.93$ \\
\srcResNet & 0 & $0.84\pm0.05$ & $3.58\pm0.12$ & $6.20\pm0.25$ & $10.66\pm0.42$ & $18.71\pm0.49$ & $25.92\pm0.50$ \\
\srcResNet & 2 & $1.61\pm0.01$ & $5.80\pm0.20$ & $9.31\pm0.43$ & $13.86\pm0.62$ & $22.38\pm0.33$ & $30.22\pm1.05$ \\
\srcResNet & 4 & $1.31\pm0.02$ & $5.38\pm0.07$ & $8.69\pm0.06$ & $13.75\pm0.47$ & $22.84\pm0.58$ & $30.38\pm0.23$ \\
\srcResNet & 8 & $1.24\pm0.10$ & $5.01\pm0.09$ & $8.42\pm0.41$ & $13.58\pm0.39$ & $23.13\pm0.40$ & $31.15\pm0.69$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
\srcCRADIO & 0 & $1.581\pm0.003$ & $0.770\pm0.008$ & -- & 0/3 & 34 & 0.27 \\
\srcCRADIO & 2 & $2.174\pm0.004$ & $0.043\pm0.002$ & $65.62\pm5.63$ & 2/3 & 99 & 0.78 \\
\srcCRADIO & 4 & $2.086\pm0.002$ & $0.033\pm0.004$ & $64.37\pm4.87$ & 2/3 & 120 & 0.95 \\
\srcCRADIO & 8 & $1.746\pm0.014$ & $0.017\pm0.002$ & $61.18\pm0.00$ & 1/3 & 101 & 0.80 \\
\srcDINO & 0 & $1.605\pm0.001$ & $0.747\pm0.002$ & -- & 0/3 & 28 & 0.22 \\
\srcDINO & 2 & $2.194\pm0.001$ & $0.045\pm0.001$ & $63.00\pm1.40$ & 2/3 & 124 & 0.98 \\
\srcDINO & 4 & $2.115\pm0.016$ & $0.027\pm0.003$ & $63.87\pm1.22$ & 2/3 & 162 & 1.28 \\
\srcDINO & 8 & $1.860\pm0.011$ & $0.016\pm0.002$ & $64.40\pm1.18$ & 2/3 & 124 & 0.98 \\
\srcRADIO & 0 & $1.542\pm0.000$ & $0.615\pm0.006$ & -- & 0/3 & 40 & 0.32 \\
\srcRADIO & 2 & $2.217\pm0.006$ & $0.040\pm0.002$ & $73.92\pm0.89$ & 2/3 & 102 & 0.80 \\
\srcRADIO & 4 & $2.079\pm0.009$ & $0.017\pm0.001$ & $69.82\pm0.00$ & 1/3 & 116 & 0.91 \\
\srcRADIO & 8 & $1.785\pm0.050$ & $0.012\pm0.001$ & $69.58\pm2.24$ & 2/3 & 108 & 0.85 \\
\srcSigLIP & 0 & $1.589\pm0.001$ & $0.739\pm0.003$ & -- & 0/3 & 16 & 0.13 \\
\srcSigLIP & 2 & $2.195\pm0.009$ & $0.043\pm0.002$ & $64.09\pm0.00$ & 1/3 & 102 & 0.80 \\
\srcSigLIP & 4 & $2.082\pm0.025$ & $0.031\pm0.003$ & $65.05\pm0.60$ & 2/3 & 126 & 0.99 \\
\srcSigLIP & 8 & $1.749\pm0.009$ & $0.019\pm0.001$ & $62.59\pm0.00$ & 1/3 & 92 & 0.73 \\
\srcSAM & 0 & $1.543\pm0.002$ & $0.451\pm0.025$ & -- & 0/3 & 0 & 0.00 \\
\srcSAM & 2 & $2.237\pm0.007$ & $0.033\pm0.002$ & -- & 0/3 & 16 & 0.13 \\
\srcSAM & 4 & $2.115\pm0.031$ & $0.017\pm0.002$ & -- & 0/3 & 8 & 0.06 \\
\srcSAM & 8 & $1.840\pm0.038$ & $0.007\pm0.001$ & -- & 0/3 & 7 & 0.06 \\
\srcResNet & 0 & $1.521\pm0.001$ & $0.708\pm0.012$ & -- & 0/3 & 23 & 0.18 \\
\srcResNet & 2 & $2.149\pm0.007$ & $0.043\pm0.002$ & $63.46\pm0.00$ & 1/3 & 75 & 0.59 \\
\srcResNet & 4 & $2.058\pm0.020$ & $0.024\pm0.003$ & $59.62\pm4.16$ & 2/3 & 93 & 0.73 \\
\srcResNet & 8 & $1.763\pm0.009$ & $0.015\pm0.002$ & $56.71\pm0.81$ & 2/3 & 129 & 1.02 \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
\srcVG & \srcEGTR & sgdet & 26,446 & 183,642 & 160,517 & 56,597 & 87.41 & 30.82 \\
\srcVG & \srcKERN & sgdet & 26,446 & 183,642 & 124,232 & 111,849 & 67.65 & 60.91 \\
\srcVG & \srcKERN & sgcls & 26,446 & 183,642 & 183,642 & 183,642 & 100.00 & 100.00 \\
\srcVG & \srcSGTR & sgdet & 26,446 & 183,642 & 140,764 & 32,371 & 76.65 & 17.63 \\
\srcOI & \srcEGTR & sgdet & 1,813 & 4,951 & 4,689 & 2,527 & 94.71 & 51.04 \\
\srcOI & \srcSGTR & sgdet & 1,813 & 4,951 & 4,462 & 469 & 90.12 & 9.47 \\
\srcPSG & \srcMotifs & sgdet & 1,000 & 6,429 & 3,707 & 3,707 & 57.66 & 57.66 \\
\srcPSG & \srcVCTree & sgdet & 1,000 & 6,429 & 3,707 & 3,707 & 57.66 & 57.66 \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
\srcVG & \srcEGTR & sgdet & Both match & 35,680 & 73.18 [72.51, 73.89] & 97.18 [96.93, 97.42] \\
\srcVG & \srcEGTR & sgdet & One differs & 18,265 & 63.59 [62.54, 64.59] & 93.56 [93.05, 94.02] \\
\srcVG & \srcEGTR & sgdet & Both differ & 2,652 & 49.10 [46.57, 51.77] & 83.30 [81.21, 85.31] \\
\srcVG & \srcKERN & sgdet & Both match & 14,845 & 73.15 [72.11, 74.17] & 97.88 [97.61, 98.15] \\
\srcVG & \srcKERN & sgdet & One differs & 50,591 & 64.25 [63.59, 64.88] & 93.17 [92.84, 93.49] \\
\srcVG & \srcKERN & sgdet & Both differ & 46,413 & 52.55 [51.88, 53.21] & 86.66 [86.19, 87.08] \\
\srcVG & \srcKERN & sgcls & Both match & 87,188 & 72.08 [71.53, 72.58] & 96.82 [96.64, 96.99] \\
\srcVG & \srcKERN & sgcls & One differs & 74,136 & 57.91 [57.33, 58.46] & 90.53 [90.20, 90.85] \\
\srcVG & \srcKERN & sgcls & Both differ & 22,318 & 42.19 [41.29, 43.14] & 81.56 [80.89, 82.26] \\
\srcVG & \srcSGTR & sgdet & Both match & 17,473 & 52.61 [51.54, 53.70] & 78.76 [77.81, 79.63] \\
\srcVG & \srcSGTR & sgdet & One differs & 12,060 & 42.20 [41.02, 43.38] & 71.35 [70.19, 72.56] \\
\srcVG & \srcSGTR & sgdet & Both differ & 2,838 & 36.26 [33.76, 38.78] & 66.95 [64.57, 69.24] \\
\srcOI & \srcEGTR & sgdet & Both match & 1,076 & 87.36 [85.13, 89.56] & 99.91 [99.71, 100.00] \\
\srcOI & \srcEGTR & sgdet & One differs & 1,114 & 87.16 [84.60, 89.59] & 98.47 [97.65, 99.20] \\
\srcOI & \srcEGTR & sgdet & Both differ & 337 & 81.90 [76.61, 86.71] & 97.33 [95.11, 99.14] \\
\srcOI & \srcSGTR & sgdet & Both match & 199 & 67.84 [61.19, 74.40] & 95.48 [92.12, 98.41] \\
\srcOI & \srcSGTR & sgdet & One differs & 204 & 58.82 [52.31, 65.66] & 97.55 [95.26, 99.49] \\
\srcOI & \srcSGTR & sgdet & Both differ & 66 & 59.09 [44.93, 72.58] & 90.91 [83.08, 98.25] \\
\srcPSG & \srcMotifs & sgdet & Both match & 2,804 & 54.28 [52.33, 56.23] & 91.16 [89.95, 92.33] \\
\srcPSG & \srcMotifs & sgdet & One differs & 836 & 41.87 [38.11, 45.59] & 88.28 [85.64, 90.74] \\
\srcPSG & \srcMotifs & sgdet & Both differ & 67 & 55.22 [42.65, 67.12] & 95.52 [89.86, 100.00] \\
\srcPSG & \srcVCTree & sgdet & Both match & 2,831 & 54.22 [52.22, 56.13] & 92.33 [91.13, 93.44] \\
\srcPSG & \srcVCTree & sgdet & One differs & 816 & 46.69 [42.82, 50.59] & 89.71 [87.39, 91.78] \\
\srcPSG & \srcVCTree & sgdet & Both differ & 60 & 56.67 [41.07, 70.77] & 96.67 [91.38, 100.00] \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
\srcPyMotifs & Key-node mask & 1,999 & 43.00 & 43.52 & 43.15 & 42.26 & 19.67 & \keynum{23.33 [21.74, 24.83]} \\
\srcPyMotifs & Random-node mask & 1,999 & 43.00 & 42.95 & 42.96 & 42.26 & 28.90 & 14.10 [13.01, 15.20] \\
\srcPyMotifs & Unrelated-node mask & 1,875 & 43.00 & 42.24 & 42.81 & 42.17 & 36.39 & \keynum{5.79 [4.66, 6.90]} \\
\srcPyMotifs & On-manifold replacement & 1,995 & 43.00 & 42.97 & 37.52 & 29.51 & 25.70 & 17.29 [16.07, 18.45] \\
\srcPyMotifs & Color jitter & 1,999 & 43.00 & 43.06 & 43.23 & 43.39 & 43.22 & -0.23 [-0.81, 0.36] \\
\srcPyTransformer & Key-node mask & 1,999 & 62.49 & 62.48 & 61.92 & 60.12 & 20.02 & \keynum{42.46 [40.87, 44.12]} \\
\srcPyTransformer & Random-node mask & 1,999 & 62.49 & 62.50 & 62.16 & 61.40 & 35.78 & 26.70 [25.49, 27.95] \\
\srcPyTransformer & Unrelated-node mask & 1,875 & 62.49 & 61.99 & 61.80 & 61.55 & 50.44 & \keynum{11.54 [10.33, 12.77]} \\
\srcPyTransformer & On-manifold replacement & 1,995 & 62.49 & 61.57 & 52.96 & 37.74 & 32.07 & 30.34 [29.02, 31.59] \\
\srcPyTransformer & Color jitter & 1,999 & 62.49 & 62.42 & 62.36 & 62.22 & 62.21 & 0.27 [-0.05, 0.65] \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
\srcPyMotifs & SGCLS & Both match & 6,050 & 49.92 [47.77, 52.24] & 87.42 [86.02, 88.77] \\
\srcPyMotifs & SGCLS & One differs & 7,659 & 42.45 [40.66, 44.27] & 82.19 [80.97, 83.43] \\
\srcPyMotifs & SGCLS & Both differ & 3,254 & 35.00 [32.26, 37.91] & 72.07 [69.68, 74.46] \\
\srcPyMotifs & SGDET & Both match & 3,555 & 51.48 [48.68, 54.34] & 87.57 [85.82, 89.33] \\
\srcPyMotifs & SGDET & One differs & 5,037 & 40.48 [38.26, 42.65] & 75.74 [73.77, 77.60] \\
\srcPyMotifs & SGDET & Both differ & 1,341 & 32.07 [27.56, 36.85] & 70.99 [66.75, 75.04] \\
\srcPyTransformer & SGCLS & Both match & 8,844 & 71.30 [69.62, 72.96] & 96.21 [95.54, 96.84] \\
\srcPyTransformer & SGCLS & One differs & 6,575 & 60.76 [58.79, 62.60] & 91.65 [90.65, 92.56] \\
\srcPyTransformer & SGCLS & Both differ & 1,544 & 43.26 [38.99, 47.39] & 82.19 [79.34, 84.97] \\
\srcPyTransformer & SGDET & Both match & 3,555 & 71.53 [69.17, 73.86] & 96.12 [94.89, 97.08] \\
\srcPyTransformer & SGDET & One differs & 5,037 & 63.59 [61.55, 65.50] & 93.09 [92.07, 94.02] \\
\srcPyTransformer & SGDET & Both differ & 1,341 & 57.72 [53.60, 61.76] & 90.83 [88.47, 92.99] \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
\srcMotifs & 115 & 1,296 & 88.87 [85.66, 91.82] & \keynum{1.74 [0.00, 4.85]} & 96.60 [95.32, 97.77] & $-4.26$ [$-6.85$, $-1.78$] & \keynum{$-51.30$ [$-72.42$, $-31.24$]} & $-0.08$ [$-1.65$, 1.46] \\
\srcPyTransformer & 191 & 1,220 & 86.18 [82.17, 89.79] & \keynum{1.05 [0.00, 2.89]} & 99.51 [99.08, 99.84] & $-5.96$ [$-10.11$, $-2.57$] & \keynum{$-57.89$ [$-70.57$, $-43.60$]} & 2.13 [1.00, 3.46] \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
\srcVG & \srcEGTR & 4.86 & 13.77 & 18.93 & 24.31 & \keynum{30.88} & 34.99 \\
\srcVG & \srcRelTR & 3.99 & 11.63 & 16.20 & 20.90 & 25.63 & 27.75 \\
\srcVG & \srcSGTR & 3.97 & 10.79 & 14.61 & 18.69 & 23.21 & 26.22 \\
\srcVG & \srcKERN & 4.32 & 12.21 & 16.82 & 21.63 & 26.93 & 29.65 \\
\srcVG & \srcBGNN & 3.94 & 12.04 & 17.35 & 23.10 & 30.27 & 34.98 \\
\srcOI & \srcEGTR & 36.32 & 63.03 & 69.33 & 74.28 & \keynum{78.38} & 81.42 \\
\srcOI & \srcSGTR & 34.34 & 58.63 & 64.50 & 68.06 & 71.76 & 72.75 \\
\srcPSG & \srcMotifs & 6.64 & 16.29 & 19.87 & 22.94 & 24.73 & 25.23 \\
\srcPSG & \srcVCTree & 6.85 & 16.87 & 20.48 & 22.98 & 24.98 & 25.40 \\
\srcPSG & \srcPSGFormer & 5.76 & 10.51 & 13.48 & 16.43 & 19.79 & 22.48 \\
\srcPSG & \srcPSGTR & 7.34 & 19.95 & 26.31 & 32.50 & \keynum{37.76} & 38.64 \\
\bottomrule
\end{tabular}
\end{table*}
...
Data & Model & @1 & @5 & @10 & @20 & @50 & @100 \\
\midrule
\srcVG & \srcEGTR & 0.94 & 2.97 & 4.32 & 5.83 & 8.50 & 10.75 \\
\srcVG & \srcRelTR & 0.68 & 2.57 & 4.14 & 5.99 & 8.91 & 10.65 \\
\srcVG & \srcSGTR & 0.94 & 3.05 & 4.54 & 7.07 & \keynum{11.64} & 14.57 \\
\srcVG & \srcKERN & 0.88 & 2.63 & 3.73 & 5.21 & 6.90 & 7.99 \\
\srcVG & \srcBGNN & 1.93 & 4.49 & 6.30 & 8.06 & 11.04 & 13.12 \\
\srcOI & \srcEGTR & 15.16 & 35.87 & 40.98 & 44.56 & 51.46 & 54.59 \\
\srcOI & \srcSGTR & 14.40 & 36.13 & 45.89 & 49.76 & \keynum{54.94} & 55.66 \\
\srcPSG & \srcMotifs & 2.91 & 7.62 & 8.90 & 10.08 & 10.93 & 11.15 \\
\srcPSG & \srcVCTree & 2.98 & 8.19 & 9.72 & 10.77 & 11.40 & 11.55 \\
\srcPSG & \srcPSGFormer & 2.39 & 6.36 & 8.75 & 11.43 & 14.50 & 17.89 \\
\srcPSG & \srcPSGTR & 2.90 & 10.43 & 13.74 & 17.62 & \keynum{20.64} & 20.95 \\
\bottomrule
\end{tabular}
\end{table*}
...
Data & Model & @1 & @5 & @10 & @20 & @50 & @100 \\
\midrule
\srcVG & \srcEGTR & 0.00 & 0.02 & 0.03 & 0.11 & 0.16 & 0.29 \\
\srcVG & \srcRelTR & 0.07 & 0.19 & 0.33 & 0.67 & 1.43 & 1.84 \\
\srcVG & \srcSGTR & 0.07 & 0.20 & 0.52 & 1.16 & \keynum{2.31} & 3.51 \\
\srcVG & \srcKERN & 0.00 & 0.00 & 0.00 & 0.02 & 0.02 & 0.02 \\
\srcVG & \srcBGNN & 0.00 & 0.00 & 0.00 & 0.00 & 0.06 & 0.09 \\
\srcOI & \srcEGTR & 0.00 & 6.25 & 6.25 & 11.25 & \keynum{11.25} & 12.50 \\
\srcOI & \srcSGTR & 0.00 & 0.00 & 2.50 & 10.00 & 10.00 & 10.00 \\
\srcPSG & \srcMotifs & 0.32 & 0.64 & 0.64 & 1.44 & 1.71 & 2.03 \\
\srcPSG & \srcVCTree & 0.00 & 0.21 & 0.85 & 1.28 & 1.92 & 2.08 \\
\srcPSG & \srcPSGFormer & 0.00 & 0.00 & 0.00 & 0.48 & 1.76 & 3.31 \\
\srcPSG & \srcPSGTR & 0.32 & 0.59 & 1.34 & 1.34 & \keynum{2.88} & 3.21 \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
\srcVG & \srcEGTR & 26,446 & 183,642 & 90.09 & 67.85 & 61.25 & 63.43 & 41.83 & pass \\
\srcVG & \srcRelTR & 26,446 & 183,642 & 78.71 & 66.04 & 52.19 & 58.35 & 3.27 & pass \\
\srcVG & \srcSGTR & 26,446 & 183,642 & 81.21 & 64.74 & 52.74 & 61.44 & 18.14 & pass \\
\srcOI & \srcEGTR & 1,813 & 4,951 & 98.02 & 56.83 & 55.40 & 55.62 & 19.09 & pass with protocol qualification \\
\srcOI & \srcSGTR & 1,813 & 4,951 & 96.53 & 58.50 & 56.38 & 52.75 & 18.55 & pass \\
\srcPSG & \srcMotifs & 1,000 & 6,429 & 64.16 & 84.93 & 54.91 & 78.60 & 85.08 & not applicable \\
\srcPSG & \srcVCTree & 1,000 & 6,429 & 64.16 & 84.80 & 54.98 & 78.41 & 85.05 & not applicable \\
\srcPSG & \srcPSGFormer & 1,000 & 6,429 & 66.97 & 85.96 & 57.76 & 80.72 & 13.41 & not applicable \\
\srcPSG & \srcPSGTR & 1,000 & 6,429 & 77.71 & 83.08 & 65.00 & 76.19 & 16.49 & not applicable \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
\srcPyMotifs & PredCls & R & 12.20 & 34.89 & 46.99 & 56.75 & 63.71 & 65.73 \\
\srcPyMotifs & PredCls & mR & 2.19 & 6.93 & 10.53 & 14.53 & 18.68 & 20.56 \\
\srcPyMotifs & PredCls & zR & 0.07 & 0.51 & 1.19 & 2.65 & 5.47 & 7.73 \\
\srcPyMotifs & SGCls & R & 4.99 & 14.39 & 19.71 & 24.55 & 28.57 & 30.00 \\
\srcPyMotifs & SGCls & mR & 0.58 & 2.03 & 3.21 & 4.46 & 5.82 & 6.48 \\
\srcPyMotifs & SGCls & zR & 0.02 & 0.20 & 0.50 & 0.98 & 1.79 & 2.38 \\
\srcPyMotifs & SGDet & R & 1.20 & 3.87 & 6.00 & 9.08 & 14.39 & 18.87 \\
\srcPyMotifs & SGDet & mR & 0.15 & 0.49 & 0.80 & 1.27 & 2.31 & 3.26 \\
\srcPyMotifs & SGDet & zR & 0.01 & 0.01 & 0.01 & 0.06 & 0.24 & 0.44 \\
\srcPyTransformer & PredCls & R & 13.33 & 36.72 & 48.94 & 58.50 & \keynum{65.12} & 66.94 \\
\srcPyTransformer & PredCls & mR & 2.09 & 6.33 & 9.60 & 13.27 & \keynum{17.15} & 18.68 \\
\srcPyTransformer & PredCls & zR & 0.00 & 0.10 & 0.23 & 0.73 & 2.45 & 4.25 \\
\srcPyTransformer & SGCls & R & 9.33 & 23.76 & 30.78 & 36.03 & \keynum{39.39} & 40.22 \\
\srcPyTransformer & SGCls & mR & 1.17 & 3.67 & 5.40 & 7.29 & \keynum{9.28} & 10.01 \\
\srcPyTransformer & SGCls & zR & 0.00 & 0.02 & 0.06 & 0.22 & 0.51 & 0.71 \\
\srcPyTransformer & SGDet & R & 3.72 & 11.47 & 16.58 & 22.14 & \keynum{28.88} & 32.90 \\
\srcPyTransformer & SGDet & mR & 0.75 & 2.48 & 3.82 & 5.21 & \keynum{7.26} & 8.84 \\
\srcPyTransformer & SGDet & zR & 0.01 & 0.21 & 0.37 & 0.56 & 1.15 & 2.01 \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
\srcMotifs & PredCls & 65.18/63.71/-1.47/P & 14.79/18.68/+3.89/X \\
\srcMotifs & SGCls & 38.92/28.57/-10.35/X & 8.28/5.82/-2.46/X \\
\srcMotifs & SGDet & 32.78/14.39/-18.39/X & 6.75/2.31/-4.44/X \\
\srcPyTransformer & PredCls & 65.55/65.12/-0.43/P & 16.30/17.15/+0.85/P \\
\srcPyTransformer & SGCls & 40.18/39.39/-0.79/P & 10.09/9.28/-0.81/P \\
\srcPyTransformer & SGDet & 33.04/28.88/-4.16/X & 8.13/7.26/-0.87/P \\
\bottomrule
\end{tabular}
\end{minipage}
\end{table*}
...
\begin{table*}[!t]
\caption{Complete Experiment V validation results for \srcTDEMotifs. All values are percentages and $\Delta$ is relative to the shared frozen baseline: object Top-1 75.011, ECE 17.871, SGCls R@50 30.206, and SGCls mR@50 18.841. All runs complete three epochs; the selected epoch and strict gate outcome are retained, including failed seeds.}
...
\begin{table*}[!t]
\caption{Entity-aligned full \srcVG\ test of the selected supervised seed-17 checkpoint against its frozen \srcTDEMotifs\ baseline. The evaluation covers all 26,446 images. Values are percentages and the test is not a selection criterion.}
...
\begin{table*}[!t]
\caption{Complete per-seed frozen transfer for \srcTDEMotifs\ in Experiment V. Values and paired image-bootstrap intervals are percentages. \srcGQA\ retains 604/1,000 images, 14.01\% of objects, and 3.47\% of relations; \srcVRD\ retains 770/955 images, 46.04\% of objects, and 31.56\% of relations. Baseline object/triplet/EIDR are 62.32/23.63/55.70 on GQA and 54.43/7.91/71.07 on VRD. Predicate Hit@1 is 51.92 on GQA and 26.02 on VRD before and after tuning.}
...
\midrule
\srcGQA & Sup./17 & 70.89 & +8.57 [7.13, 10.10] & 51.92 & 29.51 & +5.88 [4.31, 7.47] & 45.52 \\
\srcGQA & Sup./23 & 70.89 & +8.57 [7.03, 10.11] & 51.92 & 29.51 & +5.88 [4.30, 7.63] & 45.52 \\
\srcGQA & Sup./31 & \keynum{73.24} & \keynum{+10.92 [9.17, 12.65]} & 51.92 & \keynum{30.85} & \keynum{+7.22 [5.43, 9.18]} & \keynum{43.13} \\
\srcGQA & Grd./17 & 70.89 & +8.57 [7.13, 10.10] & 51.92 & 29.51 & +5.88 [4.31, 7.47] & 45.52 \\
\srcGQA & Grd./23 & 70.89 & +8.57 [7.03, 10.11] & 51.92 & 29.51 & +5.88 [4.30, 7.63] & 45.52 \\
\srcGQA & Grd./31 & 70.93 & +8.62 [7.11, 10.12] & 51.92 & 29.57 & +5.94 [4.34, 7.64] & 45.46 \\
\midrule
\srcVRD & Sup./17 & 60.56 & +6.13 [5.07, 7.08] & 26.02 & 9.24 & +1.33 [0.67, 2.00] & 65.57 \\
\srcVRD & Sup./23 & 60.59 & +6.16 [5.13, 7.21] & 26.02 & 9.28 & +1.37 [0.69, 2.08] & 65.53 \\
\srcVRD & Sup./31 & \keynum{61.14} & \keynum{+6.71 [5.71, 7.78]} & 26.02 & \keynum{9.45} & \keynum{+1.54 [0.86, 2.24]} & \keynum{65.03} \\
\srcVRD & Grd./17 & 60.59 & +6.16 [5.12, 7.11] & 26.02 & 9.28 & +1.37 [0.71, 2.05] & 65.53 \\
\srcVRD & Grd./23 & 60.59 & +6.16 [5.13, 7.21] & 26.02 & 9.28 & +1.37 [0.69, 2.08] & 65.53 \\
\srcVRD & Grd./31 & 60.59 & +6.16 [5.15, 7.14] & 26.02 & 9.28 & +1.37 [0.73, 2.08] & 65.53 \\
\bottomrule
\end{tabular}
\end{table*}
...
\section{Reference Execution Environment}
\label{app:environment}

Formal runs were executed on a Linux Mint 22.2 server (kernel
6.14.0-36-generic) with an Intel Xeon Silver 4210R CPU, 125\,GiB of system
memory, and two NVIDIA GeForce RTX 3090 GPUs with 24,576\,MiB each. The host
used NVIDIA driver 580.95.05, CUDA toolkit 12.0.140, and Python 3.12.3.
Historical SGG adapters retained their own isolated framework and CUDA
environments; the artifact records those per-run locks, so the host versions
are orchestration metadata rather than a claim that all models share one
runtime.
